...
int main(){
	...
	std::cout<<"test for the copy constructor"<<'\n';
	Heap tourney4("players.txt");
	assert(isHeap(tourney4.getArray(),0));
	assert(tourney4.size() == 17);
	Heap copyTourney(tourney4);
	assert(isHeap(copyTourney.getArray(),0));
	assert(copyTourney.size() == 17);
	tourney4.getPlayer();
	assert(copyTourney.size() == 17);

	std::cout<<"dealing with an emptyFile"<<'\n';
	Heap tourney5("players3.txt");
	assert(tourney5.size() == 0);
